# TRIKDIS Documentation Writer’s Guide

This guide explains how to **edit, add, and publish TRIKDIS documentation** on docs.trikdis.com.

It is intended for documentation owners and contributors who maintain product manuals and related pages.
 The guide focuses on **safe, practical workflows** using Markdown, Typora, GitHub Desktop, and MkDocs, without requiring software development knowledge.
...
2. [Creating New Pages and Navigation](#creating-new-pages-and-navigation)
...
6. [GitHub-style Alerts](#github-style-alerts)
7. [Tables](#tables)
8. [Images](#images)
9. [Typora Setup](#typora-setup)
...
## Creating New Pages and Navigation

### 1. Create the new page
Create a new `.md` file in the appropriate folder under `docs/`.

Use Typora for writing.

Use clear file names (lowercase, hyphens, no spaces).

### 2. Decide how the page should be shown on the website
A new page must be reachable from the website navigation.

There are two valid ways to do this:

- Add the page to the main menu by updating `mkdocs.yml` (recommended for new manuals or main sections).
- Link the page from an existing page that already appears in the menu (recommended for subtopics, examples, appendices, or FAQs).

Pages that are not linked anywhere will not be visible on the website.

### 3. Editing `mkdocs.yml` (navigation file)
`mkdocs.yml` controls the navigation and structure of the documentation website.

Open `mkdocs.yml` using VS Code.

Do not edit this file in Typora.

Be careful with indentation — YAML is space-sensitive.

Save the file, then commit and push as usual.

### 4. When `mkdocs.yml` needs to be updated
When a new manual is added.

When a new main page or section should appear in the website menu.

### Guideline
Use `mkdocs.yml` for main manuals and sections.

Use page-to-page links for subtopics, examples, appendices, or FAQs.

If unsure whether a page belongs in the menu, ask before pushing changes.
