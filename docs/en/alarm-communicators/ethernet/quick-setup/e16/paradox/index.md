# Paradox SP(+)/MG(+) with E16 quick setup

Short steps to connect the E16 communicator to Paradox SP/SP+/MG/MG+ panels, configure E16 for IP reporting, and add the system to Protegus2. Use this together with the full E16 manual for all other settings.
...
- Paradox SP/SP+/MG/MG+ panel with keypad access.
- `EX-CRP2.4` cable for the Paradox serial connection.
...
Connect E16 to the Paradox serial connector with the `EX-CRP2.4` cable and power the communicator from the panel:

<img alt="E16 Paradox panel connection diagram" src="../images/paradox.webp" style="width:5.2in;max-width:100%;height:auto;" />

## Panel programming

Paradox panels do not need extra programming for event reading. Program the panel only if you want direct control from Protegus2:

1. Enter installer programming on the keypad.
2. Open section `911`.
3. Enter the 4-digit **PC download password**.
4. In TrikdisConfig, make sure the same password is entered in **System settings**.
...
4. If you prefer keyswitch control instead of direct control, leave **Remote Arm/Disarm** unchecked and configure `PGM1` in Protegus2 after wiring the E16 output to the panel keyswitch zone.
