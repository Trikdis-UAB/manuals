# Texecom with E16 quick setup

Short steps to connect the E16 communicator to Texecom Premier and Premier Elite panels, configure E16 for IP reporting, and add the system to Protegus2. Use this together with the full E16 manual for all other settings.
...
- Texecom Premier / Premier Elite panel with installer access.
- Texecom EX-CRP4 cable for serial connection.
...
Use the Texecom EX-CRP4 cable (ordered separately) and connect the panel to E16 as shown below:

| E16 terminal | Texecom EX-CRP4 wire | Notes |
| --- | --- | --- |
| `+DC` | `R` (red) | `+12V` supply |
| `-DC` | `B` (black) | Panel ground |
| `CLK` | `BL` (blue) | Serial bus |
| `DATA` | `W` (white) | Serial bus |

<img alt="E16 Texecom panel connection diagram" src="../images/texecom.webp" style="width:5.2in;max-width:100%;height:auto;" />

## Panel programming

Texecom panels must be programmed for both event reading and remote control.

1. In **Wintex**, open **Communication Options** and go to the **Options** tab.
2. Enter the 4-digit **UDL passcode**.
3. Make sure the **UDL passcode** matches the **Security panel PC download password** entered in E16 **System settings** when **Remote Arm/Disarm** is enabled.
...
6. Press **[Yes]** and exit programming mode by pressing **[Menu]**.
