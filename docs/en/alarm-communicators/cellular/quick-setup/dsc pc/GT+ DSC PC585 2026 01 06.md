# DSC PowerSeries with GT/GT+/GET quick setup

Short wiring and programming steps to connect the GT/GT+/GET communicator to DSC PowerSeries panels (PC585, PC1404, PC1565, PC1616, PC1832, PC1864, PC5020) using KeyBus, then enroll the system in Protegus2. Use this alongside the full manuals for all other settings. (Terminal labels differ slightly between GT/GT+/GET, but the connections are the same.) 

!!! caution
    Install and service only by qualified personnel. Disconnect power before wiring. Unauthorized changes void warranty.

## Prerequisites

1. GT/GT+/GET firmware 1.21, SIM inserted, PIN disabled, data plan active.
1. DSC PowerSeries panel (PC585, PC1404, PC1565, PC1616, PC1832, PC1864, PC5020) with keypad access (installer code available).
1. CMS account number if reporting to CMS.
1. Protegus2 company/installer account and communicator IMEI.

## Wiring

Follow the schematic below to connect the communicator to the panel: 

| **GT/GT+/GET terminal** | **DSC panel** | **Notes**              |
| ----------------------- | ------------- | ---------------------- |
| +12V DC/-12V DC         | +AUX/-AUX     | Power the communicator |
| CLK/DATA                | YEL/GRN       | KeyBus                 |


<img src="../GT+ dsc pc585 prijungimo schema ENG 2026 01 06.webp" alt="GT+ dsc pc585 prijungimo schema ENG 2026 01 06" class="GT+ dsc pc585 prijungimo schema ENG 2026 01 06">


**DSC panels do not need to be programmed**.

## Add system to Protegus2



<div class="steps-grid">
  <div class="step-card">
        <strong>Step 1.</strong> Tap <strong>Add new system</strong>.
        <img src="../GT+ dsc pc585 1 ENG 2026 01 06.webp" alt="Add new system">
  </div>
  
 
  <div class="step-card">
        <strong>Step 2.</strong> Enter the communicator <strong>IMEI</strong>, tap <strong>Next</strong>.    
        <img src="../GT+ dsc pc585 2 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 3.</strong> Select security company. 
        <img src="../GT+ dsc pc585 3 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 4.</strong> Choose <strong>DSC</strong>.
        <img src="../GT+ dsc pc585 4 ENG 2026 01 06.webp" alt="Add new system">
  </div>
  

  <div class="step-card">
        <strong>Step 5.</strong> Choose <strong>PC 585 (PC1404, PC1565, PC1616, PC1832, PC1864, PC5020)</strong>.
        <img src="../GT+ dsc pc585 5 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 6.</strong> Enter <strong>Object ID</strong> and <strong>Module ID</strong>, tap <strong>Next</strong>.
        <img src="../GT+ dsc pc585 6 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 7.</strong> Wait while data is written.
        <img src="../GT+ dsc pc585 7 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 8.</strong> Tap <strong>Add to Protegus2</strong>.
        <img src="../GT+ dsc pc585 8 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 9.</strong> Enter system <strong>Name</strong>, tap <strong>Next</strong>.
        <img src="../GT+ dsc pc585 9 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 10.</strong> Tap <strong>Skip</strong>.
        <img src="../GT+ dsc pc585 10 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 11.</strong> Tap on system.
        <img src="../GT+ dsc pc585 11 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 12.</strong> Wait 1 minute for completion and tap <strong>Transfer</strong>.
        <img src="../GT+ dsc pc585 12 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 13.</strong> Enter the e-mail of the user to whom the installer will transfer the system. Tap <strong>Transfer</strong>.
        <img src="../GT+ dsc pc585 13 ENG 2026 01 06.webp" alt="Add new system">
  </div>


  <div class="step-card">
        <strong>Step 14.</strong> The system will appear in Protegus on the user's phone.
        <img src="../GT+ dsc pc585 14 ENG 2026 01 06.webp" alt="Add new system">
  </div>


</div>

!!! tip
    After completing the setup and installation perform a system check:

    1. Create an event:

       - by arming/disarming the system with the control panel’s keypad.
       - by triggering a zone alarm when the security system is armed.

    2. Make sure that the event arrives to the CMS (Central Monitoring Station) and the Protegus2 app.
